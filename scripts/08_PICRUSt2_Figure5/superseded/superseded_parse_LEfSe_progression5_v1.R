...
LEFSE_DIR <- file.path(
  MAIN_OUT,
  "LEfSe_MetaCyc_pathways_progression5_oldstyle_currentdata_previous127_v0.5_step2"
)

RES_FILE <- file.path(
  LEFSE_DIR,
  "LEfSe_results_MetaCyc_pathways_progression5_oldstyle_currentdata_previous127_v0.5_step2.res"
)

FEATURE_MAP_FILE <- file.path(
  LEFSE_DIR,
  "MetaCyc_pathway_feature_name_map_oldstyle_currentdata_previous127_v0.5_step2.csv"
)
...
OUT_CLEAN <- file.path(
  LEFSE_DIR,
  "FINAL_LEfSe_clean_markers_MetaCyc_pathways_oldstyle_currentdata_previous127_v0.5_step2.csv"
)
OUT_COUNTS <- file.path(
  LEFSE_DIR,
  "FINAL_LEfSe_marker_counts_by_group_MetaCyc_pathways_oldstyle_currentdata_previous127_v0.5_step2.csv"
)
OUT_TOP40 <- file.path(
  LEFSE_DIR,
  "FINAL_LEfSe_top40_by_group_MetaCyc_pathways_oldstyle_currentdata_previous127_v0.5_step2.csv"
)
OUT_SUMMARY <- file.path(
  LEFSE_DIR,
  "FINAL_summary_LEfSe_MetaCyc_pathways_oldstyle_currentdata_previous127_v0.5_step2.tsv"
)
...
clean_markers <- res %>%
  ...
  mutate(
    ...
    Description_matched = !is.na(Pathway_ID) & Pathway_ID != Feature
  ) %>%
  select(
    Class,
    Pathway_ID,
...
cat("\n============================================================\n")
cat("FINAL old-style current-data MetaCyc pathway LEfSe parsing finished\n")
cat("============================================================\n\n")
